...
from blitzecdn.api.operations import EdgeRemoval, HostRun, OriginReport, as_operation
from blitzecdn.api.requests import OriginCheckRequest
from blitzecdn.api.v2_models import Edge, EdgePatch

router = APIRouter(dependencies=[Depends(require_operator)])


@router.get("/v2/edges", response_model=list[Edge])
...
@router.get("/v2/edges/{name}", response_model=Edge)
def get_edge(name: str, control: ControlPlaneDependency) -> Edge:
    return Edge.from_domain(control.edges.get_edge(name))


@router.post("/v2/edges", response_model=Edge, status_code=status.HTTP_201_CREATED)
def add_edge(
...
@router.patch("/v2/edges/{name}", response_model=Edge)
def update_edge(
    name: str,
    patch: EdgePatch,
...
@router.delete("/v2/edges/{name}", response_model=EdgeRemoval)
def remove_edge(
    name: str,
    operator: OperatorDependency,
    control: ControlPlaneDependency,
...
@router.post("/v2/origins/check", response_model=OriginReport)
def check_origins(
