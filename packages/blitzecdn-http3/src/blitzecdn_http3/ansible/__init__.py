...
by counting ``..`` from ``__file__``. Counting is three lines and no check:
correct in a checkout and correct in an ordinary wheel, and silent in exactly
the case the check exists for — a distribution that is not unpacked, where
Ansible is handed something that is not a filesystem path at all.
"""
