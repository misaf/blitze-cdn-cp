...
import pytest
from blitzecdn_backup.adapters import TarArchive, TemporaryWorkspace
...
def test_a_restored_database_is_consistent_and_migrated(populated):
    ...
    try:
        ...
        store.close()
    assert DatabaseSchema.of(populated.database_path) is not None
...
def test_container_restore_requires_the_host_compose_wrapper(monkeypatch):
    monkeypatch.setattr(
        "blitzecdn_backup.adapters.services.Path.exists", lambda _path: True
    )
...
def test_checkout_restore_needs_no_container_lifecycle(monkeypatch):
    monkeypatch.setattr(
        "blitzecdn_backup.adapters.services.Path.exists", lambda _path: False
    )
    monkeypatch.delenv("COMPOSE_RESTORE_OFFLINE", raising=False)
    with ComposeRestoreGuard().stopped():
        pass
